...
function GradeChooseCourseComponent() {
    const {auth} = useAuth();
    ...
    const addGrade = function (rowData) {
        ...
        navigate(routes.secretaryRoutes.GradeAddComponent.path.replace(":ids", id), {replace: true});
    }
    ...
    return (
        <>
            ...
            <div className="pt-4">
                <h5 style={{color: 'white'}}>Selectează un curs, apoi adaugă notele studenților</h5>
                <hr/>
                <div className={"container"}>
                    <DataTable value={courseList} paginator rows={rowsPerPageOptionSelected}
                        ...
                        <Column field="subjectStudyPeriodCourseDetailsDto.studyYear" sortable
                                header="An studiu"></Column>
                        <Column header="Adaugă note" body={actionButtons}></Column>
                    </DataTable>
                </div>
            </div>
        </>
...
export default GradeChooseCourseComponent
